...
function assertSafeguards(campaign) {
  ...
  // Every campaign sends AS a legal entity, and inherits that entity's
  // compliance mode rather than choosing its own. A campaign that could pick
  // its own mode could pick its way out of the RUO gate.
  if (!campaign.entityId) {
    throw new Error(`Campaign "${campaign.id}" must declare the entityId it sends as.`);
  }
  const entity = getEntity(campaign.entityId);
  // The RUO check stays first and keeps its own wording: it is the original
  // safeguard, and it is the one a reader needs named when it fires.
  if (entity.posture === 'ruo_research_supply' && campaign.complianceMode !== 'research-only') {
    throw new Error(`Campaign "${campaign.id}" must use research-only compliance mode (RUO posture).`);
  }
  // General case, for entities in any other posture.
  if (campaign.complianceMode !== entity.complianceMode) {
    throw new Error(
      `Campaign "${campaign.id}" declares compliance mode "${campaign.complianceMode}" but sends as `
      + `"${entity.id}", whose mode is "${entity.complianceMode}".`
    );
  }
  return campaign;
}
...
/** Every campaign a given entity is permitted to send. */
function campaignsForEntity(entityId) {
  getEntity(entityId);
  return Object.values(CAMPAIGNS).filter(campaign => campaign.entityId === entityId);
}
