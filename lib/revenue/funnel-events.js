...
const FUNNEL_STAGES = Object.freeze([
  ...
  'reply_received',
  'qualified',
  // A booked call is a real funnel milestone and was missing entirely: the
  // prospect store has had a `meeting_booked` STAGE since it was written
  // (lib/postgres-prospect-store.js, lib/outreach-enqueue.js), but there was no
  // matching funnel EVENT, so validateEvent() rejected it as UNKNOWN_EVENT_TYPE.
  // A prospect could reach "meeting booked" and the revenue funnel could not see
  // it — the conversion walk jumped straight from qualified to offer_sent.
  'meeting_booked',
  // Booked is not held, and the gap between them is the whole story of an
  // outbound-booked calendar. No-shows are routine, so a funnel that records
  // only bookings reports the number most flattering to whoever booked them.
  // Two events means the no-show rate is a measurable quantity instead of an
  // invisible loss.
  'meeting_held',
  'offer_sent',
  'purchase_completed',
