...
const { classifyEmail } = require('../contacts/provider-ingest');
  ...
  for (const business of ordered) {
    ...
    if (!email && business.website && enrichEmail) {
      try {
        ...
        // That is a private individual's mailbox in a B2B prospect list —
        // outside the B2B-only authorization, and the kind of address that
        // earns spam complaints rather than replies. The phone and the listing
        // are still worth having, so the record is kept and only the address is
        // dropped.
        const kind = classifyEmail(candidate, business.website);
