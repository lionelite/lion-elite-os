...
// JPEG, not PNG: Instagram's content-publishing API only reliably accepts
// JPEG image URLs (Facebook/X/Bluesky accept either). Standardizing on JPEG
// keeps every platform, and every human-dropped image, on the one format
// that works everywhere.
function mediaRelativePath(piece) {
  return `content/media/${piece.date}/${piece.id}.jpg`;
}
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/images/generations', {
      ...
      body: JSON.stringify({
        ...
        size: imageSizeFor(),
        quality: 'medium',
        // Instagram requires JPEG; request it directly so no conversion
