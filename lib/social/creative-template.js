...
/**
 * Background-only prompt for the compositor path (approach B): the AI makes
 * the cinematic lion + integrated vial and CLEAN NEGATIVE SPACE, and renders
 * NO headline/paragraph text (the crisp text is composited afterward). This
 * avoids the garbled-text failure mode of asking an image model to render
 * dense typography.
 *
 * @param {boolean} [p.withVial=true]  When false, the AI renders an empty,
 *   well-lit rock pedestal in the lower-right instead of a vial — because the
 *   REAL branded vial (from lib/social/vial-registry.js) will be composited
 *   there afterward. This is the preferred path once a product has a real
 *   vial asset: the label is always correct and never AI-garbled.
 */
function buildBackgroundPrompt({ name, flair, format = 'vertical 4:5, 1080x1350', withVial = true }) {
  const foreground = withVial
    ? `A photorealistic small glass research vial with a black-and-gold Lion Elite Wellness label reading "${name.toUpperCase()}" sits naturally on a rock in the lower-right foreground, lit to match the scene.`
    : 'An empty, flat-topped rock pedestal sits in the lower-right foreground, cleanly lit with warm golden-hour light and a subtle highlight on top — a natural place for a product to rest. Leave the pedestal top clear (a product is added there later).';
  return [
    `${format} premium cinematic editorial background for Lion Elite Wellness, National-Geographic-meets-luxury-pharma.`,
    `Scene: ${flair}. Warm golden-hour light, deep blacks and charcoal, rich gold accents, sharp and photorealistic, high dynamic range.`,
    foreground,
    'Composition: keep the LEFT THIRD and a strip along the BOTTOM as clean, darker, low-detail negative space for text overlay. The lion occupies the right and center.',
    'Palette: deep black/charcoal, metallic gold (#C9A24B), warm savanna amber. Mood: premium, authoritative, editorial.',
    'IMPORTANT: render NO headline text, NO paragraph text, NO icons, NO logos, NO captions anywhere — only the photographic scene. Text is added separately.'
  ].join(' ');
}
