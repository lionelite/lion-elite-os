...
const { buildCreativePrompt, buildBackgroundPrompt, headlineFor, flairForIndex } = require('./creative-template');
...
const PEPTIDE_CATALOG = Object.freeze([
  ...
  const headline = headlineFor(entry.name);
  const pathways = pathwaysFromArea(entry.researchArea);
  const flair = flairForIndex(index);
  return {
    ...entry,
    caption: caption(entry.name, entry.researchArea, entry.note),
    headline,
    pathways,
    // Full single-prompt creative (approach A — AI renders everything).
    imagePrompt: buildCreativePrompt({ name: entry.name, headline, pathways, flair }),
    // Compositor path (approach B — AI renders the scene, text is overlaid).
    backgroundPrompt: buildBackgroundPrompt({ name: entry.name, flair })
  };
}));
