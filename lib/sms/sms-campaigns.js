...
// A campaign's compliance mode is no longer a free choice from a list — it is
// whatever its SENDING ENTITY's mode is. Wellness texts are research-only,
// Beauty texts are coaching, and neither can pick the other's ruleset by
...
function assertSafeguards(campaign) {
  ...
  if (!campaign.entityId) {
    throw new Error(`SMS campaign "${campaign.id}" must declare the entityId it sends as.`);
  }
  const entity = getEntity(campaign.entityId);
  if (campaign.complianceMode !== entity.complianceMode) {
    throw new Error(
      `SMS campaign "${campaign.id}" declares compliance mode "${campaign.complianceMode}" but ` +
      `sends as "${entity.id}", whose mode is "${entity.complianceMode}".`
    );
  }
