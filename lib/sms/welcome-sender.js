...
//   3b. the sending entity is active and has its OWN 10DLC origination number
//   4. per-recipient selection (the entity the consent was given to, consent,
//      opt-out, suppression, E.164, quiet hours in THEIR timezone,
//      once-per-lead)
...
const { assertSmsSendable } = require('../entities/registry');
...
async function runWelcomeCampaign({
  ...
  // The entity actually doing the texting must be active and have its own
  // origination number — A2P 10DLC brand registration is tied to a legal
  // entity, so borrowing another entity's number means texting under someone
  // else's carrier registration. Skipped in a dry run for the same reason
  // credentials are: selection and copy stay inspectable without them.
  if (!dryRun) {
    try {
      assertSmsSendable(campaign.entityId);
    } catch (error) {
  ...
  const { eligible, skipped } = selectSmsRecipients(candidates, {
    ...
    // Consent given to another entity is not consent for this one.
