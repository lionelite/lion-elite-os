 ...
 * @param {string} [opts.entityId] the legal entity sending. When given, a
 *   recipient whose SMS consent was given to a DIFFERENT entity is skipped.
 *   Consent is given to a company, not to an operator running several, and
 *   under TCPA that is not a matter of preference. Omitted, behaviour is
 *   unchanged — there is one sending entity today.
 */
function selectSmsRecipients(recipients = [], { now = Date.now(), localHourFor, campaignId = 'client_research_reorder_sms', entityId } = {}) {
  ...
  for (const r of recipients) {
    ...
    // A consent record that names another entity is not consent for this one.
    // A record with no entity recorded predates entity separation and belongs
    // to the entity that was sending then, so it is not treated as a mismatch.
    else if (entityId && r.smsConsentEntityId && r.smsConsentEntityId !== entityId) reason = 'consent_other_entity';
