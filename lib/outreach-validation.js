...
// Source approval moved to lib/contacts/sources.js when licensed third-party B2B
// contact data was authorized (owner decision 2026-09-22). The old inline rule was
// `Boolean(source.approved && source.url)` — it required a URL because the only
// approved origin was a page we had scraped, so every purchased record would have
// failed this check. Approval is now a provenance question, evaluated there.
const { evaluateSource } = require('./contacts/sources');
...
function buildValidationContext(prospect = {}, policy = {}, runtime = {}) {
  ...
  const sourceEvaluation = evaluateSource(source, { env: runtime.env });

  return {
    approved_source: sourceEvaluation.approved,
    ...
    diagnostics: {
      missingFields,
      // Named so a blocked record reports the missing provenance field rather
      // than a bare "approved_source failed".
      sourceType: sourceEvaluation.type,
      sourceBlockers: sourceEvaluation.blockers,
      sourceWarnings: sourceEvaluation.warnings,
