...
const CONTROL_ENV = Object.freeze({
  ...
  // No env var establishes the ad spend cap, and inventing one would be worse
  // than having no check: a gate that opens on a variable nothing else reads
  // looks like authorization while governing nothing. Per CLAUDE.md the cap is
  // an owner decision made out of band ("if no spend cap has been established,
  // Claude must obtain one"), and `lib/ads/` is plan-generation only — it states
  // outright that nothing there spends money. So this is a human decision, same
  // as HUMAN_APPROVAL.
  AD_SPEND_CAP: null,
  HUMAN_APPROVAL: null,
});
...
function controlEnabled(control, env = process.env) {
  ...
  // A control with no env var is a human decision, never auto-satisfied by
  // configuration. That covers HUMAN_APPROVAL and the ad spend cap.
