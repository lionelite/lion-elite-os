...
class PostgresProspectStore {
  ...
  async #event(client, prospectId, type, data = {}, actor = 'system') {
    await client.query(
      `INSERT INTO audit_events (event_id, prospect_id, event_type, actor, data)
       VALUES ($1,$2,$3,$4,$5::jsonb)`,
      [id('evt'), prospectId, type, actor, JSON.stringify(data)]
    );
  }
  ...
  async getDailyEmailQuota(day = dateKey()) {
    const result = await pool.query('SELECT email_sent FROM daily_usage WHERE usage_day=$1', [day]);
    const sent = Number(result.rows[0]?.email_sent || 0);
  ...
  async markQueue(queueId, status, metadata = {}, actor = 'system') {
    return withTransaction(async client => {
      ...
      if (status === 'processing' && item.channel === 'email') {
        const usage = await client.query('SELECT email_sent FROM daily_usage WHERE usage_day=CURRENT_DATE FOR UPDATE');
        const sent = Number(usage.rows[0]?.email_sent || 0);
      ...
      if (status === 'sent' && item.channel === 'email' && item.status !== 'sent') {
        const usage = await client.query(
          `INSERT INTO daily_usage (usage_day,email_sent) VALUES (CURRENT_DATE,1)
           ON CONFLICT (usage_day) DO UPDATE SET email_sent=daily_usage.email_sent+1,updated_at=NOW()
           WHERE daily_usage.email_sent < $1 RETURNING email_sent`,
  ...
  async timeline(prospectId) {
    const result = await pool.query('SELECT * FROM audit_events WHERE prospect_id=$1 ORDER BY created_at ASC', [prospectId]);
    return result.rows.map(row => ({ eventId: row.event_id, prospectId: row.prospect_id, type: row.event_type, actor: row.actor, data: row.data, createdAt: row.created_at }));
  }
