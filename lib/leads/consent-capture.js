...
function buildCapture(input = {}, context = {}) {
  ...
  // An IANA zone name, validated by asking the runtime to use it. A bogus zone
  // is dropped rather than stored, so it fails closed at send time instead of
  // silently resolving to the server's own clock.
  let timezone = clean(input.timezone, 64);
  if (timezone) {
