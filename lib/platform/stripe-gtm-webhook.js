...
const {issueLoginLink}=require('./gtm-access');
...
async function provisionCheckout(event){
  ...
  const provisioned=await db.withTransaction(async client=>{
  ...
  if(provisioned.status==='provisioned'){
    try{provisioned.access=await issueLoginLink(email,{purpose:'activation',send:true})}catch(error){provisioned.access={sent:false,error:String(error.message||error).slice(0,180)}}
  }
  return provisioned;
}
