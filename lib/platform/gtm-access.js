...
async function sendAccessEmail({email,magicUrl,subject='Your BuildPipeline access link'}){
  const apiKey=String(process.env.RESEND_API_KEY||'').trim();
  if(!apiKey)return {sent:false,reason:'RESEND_API_KEY_NOT_CONFIGURED'};
  const from=String(process.env.GTM_EMAIL_FROM||process.env.COACHING_EMAIL_FROM||'BuildPipeline <info@lionelitewellness.com>').trim();
  const html=`<!doctype html><html><body style="margin:0;background:#090b10;color:#f6f7f9;font-family:Arial,sans-serif"><div style="max-width:600px;margin:auto;padding:36px 20px"><div style="border:1px solid #282f3a;background:#0e1219;border-radius:18px;padding:30px"><div style="font-size:12px;letter-spacing:2px;color:#d3aa52;font-weight:700">BUILDPIPELINE</div><h1 style="font-size:28px">Your workspace is ready.</h1><p style="color:#b9c0c8;line-height:1.7">Use this one-time link to open BuildPipeline. It expires in 30 minutes.</p><a href="${magicUrl}" style="display:inline-block;background:#d3aa52;color:#111;text-decoration:none;font-weight:700;padding:13px 18px;border-radius:10px">Open BuildPipeline</a><p style="font-size:12px;color:#747f8c;margin-top:22px">If you did not request this link, you can ignore this email.</p></div></div></body></html>`;
  ...
  if(!r.ok)throw new Error(j.message||'BuildPipeline access email failed');
...
async function issueLoginLink(email,{purpose='login',send=true}={}){
  ...
  if(send)delivery=await sendAccessEmail({email:e,magicUrl,subject:purpose==='activation'?'Activate your BuildPipeline workspace':'Your BuildPipeline access link'});
