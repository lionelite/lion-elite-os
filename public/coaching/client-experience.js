  ...
  function eliteDashboard() {
    ...
    return `<section class="le-elite-dashboard" data-elite-dashboard>
      ...
      <div class="le-client-hub__grid">
        ...
        <button type="button" class="le-client-module le-client-module--protocol" data-elite-destination="protocol"><span class="le-client-module__icon">P</span><span class="le-client-module__body"><span class="le-client-module__top"><strong>Protocol</strong>${planStatus(protocol)}</span><small>${protocol ? `${protocol.items?.length || 0} clinician-confirmed item${protocol.items?.length === 1 ? '' : 's'}` : 'Clinician-confirmed information only.'}</small></span><span class="le-client-module__arrow">→</span></button>
